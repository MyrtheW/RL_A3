...
class MonteCarloTreeSearch:
    def __init__(
        ...
        if not self.base_pairs:
            self.base_pairs = (
                {"AU": 0.51, "GC": 0.20, "UG": 0.10}
                if self.RNA
                else {"AT": 0.51, "GC": 0.20, "TG": 0.10}
            )  # and woble base pairs "GA"and "UG"
        # standard
        ...
        self.decaying_cp = decaying_cp
        self.cp_decay = cp_decay
        self.cp_min = cp_min
    ...
    def run(self):
        ...
        while self.resources_left():  # tot target sequence is found]
            ...
            if self.c_parameter > self.cp_min and self.decaying_cp == True:
                self.c_parameter *= self.cp_decay
            if similarity >= 1:
                return (
                    sequence,
                    similarity,
        ...
        return (
            self.structure_memory[highest_similarity][0],
            highest_similarity,
            self.structure_memory[highest_similarity][1],
            self.number_evaluations,
        )

    def resources_left(self):
        return self.number_evaluations < self.max_evaluations - 2 * (
            self.max_local_search_evaluations + 1
        )
        ...
        try:
            if len(node.sequence_dict.keys()) != 1:
                position = random.sample(
                    self.all_positions - set(node.sequence_dict.keys()), 1
                )[0]
            else:
            ...
            node.visits = float(
                "Inf"
            )  # we cannot extend a leaf node with a full sequence, so we make sure it is not visited again
            print(
                "We are trying to extend a leaf node with a full sequence. It is suboptimal that we end in this leaf"
            )
            return
    ...
    def rollout(self, node):
        ...
        (
            similarity,
            i_non_aligned_nucleotides,
            sequence,
            predicted_structure,
        ) = self.evaluate(sequence_dict_rollout)

        # evt. local search, maintaing  positions of that node. while similarity !=  1.
        local_search_evaluations = 0
        while (
            similarity != 1
            and local_search_evaluations < self.max_local_search_evaluations
        ):
            new_sequence_dict, stop_local_search = self.mutate(
                sequence_dict_rollout,
                predicted_structure,
                i_non_aligned_nucleotides,
                node.sequence_dict,
            )
            ...
            (
                new_similarity,
                new_i_non_aligned_nucleotides,
                new_sequence,
                new_predicted_structure,
            ) = self.evaluate(new_sequence_dict)
    ...
    def rollout_policy(self, sequence_dict_rollout):
        ...
        while len(sequence_dict_rollout) < len(self.all_positions):
            if len(sequence_dict_rollout.keys()) != 1:
                position = random.sample(
                    self.all_positions - set(sequence_dict_rollout.keys()), 1
                )[0]
            else:
                positions = self.all_positions.copy()
                ...
                position = random.sample(positions, 1)[0]
            if type(position) == tuple:
                base_pair = np.random.choice(
                    a=list(self.base_pairs.keys()),
                    p=list(self.base_pairs.values())
                    / np.sum(list(self.base_pairs.values())),
                )[:: random.choice([1, -1])]
            else:
    ...
    def evaluate(self, sequence_dict_rollout):
        ...
        similarity, i_non_aligned_nucleotides = self.structure_similarity(
            predicted_structure
        )
    ...
    def best_child(self, node, final=False):
        choices_weights = [
            (child.similarity / child.visits)
            + self.c_parameter * np.sqrt((np.log(node.visits) / child.visits))  # UCT
            for child in node.children
        ]  # initialize visits with 1, otherwise division by zero
        if final:
            choices_weights = [
                (child.similarity / child.visits) for child in node.children
            ]
    ...
    def call_RNAfold(self, sequence):
        ...
        return str(
            subprocess.run(command, stdout=subprocess.PIPE).stdout.split()[1], "utf-8"
        )  # capture_output=True for newer version
            ...
            if self.target_structure[i] != predicted_structure[i]:
                if i not in self.all_positions:
                    i = [
                        ...
                        if type(item) == tuple and i in item
                    ][0]
                i_non_aligned_nucleotides.add(i)
        similarity = (
            len(self.target_structure) - len(i_non_aligned_nucleotides)
    ...
    def get_sequence(self, sequence_dict):
        ...
        return "".join(
            [value for (key, value) in sorted(ordered_sequence_dict.items())]
        )
    ...
    def mutate(
        ...
        predicted_structure,
        i_non_aligned_nucleotides,
        sequence_dict_original={},
    ):
        try:
            if len(sequence_dict_original.keys()) != 1:
                position = random.choice(
                    list(i_non_aligned_nucleotides - set(sequence_dict_original.keys()))
                )
            else:
        ...
        if type(position) == tuple:
            ...
            alternatives = list(self.base_pairs.keys()) + [
                key[::-1] for key in self.base_pairs.keys()
            ]
    ...
    def run_local_search(self):
        ...
        sequence_dict, similarity, predicted_structure, i_non_aligned_nucleotides = (
            self.rollout_policy({}),
            0,
            None,
            self.all_positions,
        ...
        while self.resources_left():
            new_sequence_dict, stop_local_search = self.mutate(
                sequence_dict, predicted_structure, i_non_aligned_nucleotides
            )
            (
                new_similarity,
                new_i_non_aligned_nucleotides,
...
if __name__ == "__main__":
    ...
    output = (
        mcts_object.run()
    )  # use .run_local_search if you want to run local search instead.
    print(output)
